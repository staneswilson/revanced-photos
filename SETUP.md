...
| Install on phone: `App not installed as app isn't compatible` + CI log shows `[abiInventory] ... contains no arm64-v8a` | APKPure ships recent Photos as 32-bit-only; modern 64-bit-only Android (Pixel 7+, etc.) refuses to install it. Pin `GPHOTOS_VERSION` to an older release that bundled all ABIs in one universal APK (try `7.21.0.737764319`; list candidates with `apkeep -l -a com.google.android.apps.photos -d apk-pure ./tmp`). Iterate older until `[abiInventory]` reports `arm64-v8a`. |
